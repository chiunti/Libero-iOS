...
- (IBAction)btnSavePressed:(id)sender {
    ...
    currentPerfil[@"nombre"] = self.txtNombre.text;
    currentPerfil[@"disciplina"] = maDisciplina [[self.pvDisciplina  selectedRowInComponent:0]];
    currentPerfil[@"fbId"] = fbUser.objectID;
    ...
    currentPerfil[@"imagen"] = imageFile;
    
    [self saveDataWithObject:currentPerfil];
}
...
-(void)saveDataWithObject:(PFObject *)testObject
{
    ...
    [testObject saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        ...
        if (!error) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Actualización completa" message:@"Perfil guardado" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alert show];
            
            // Notify table view to reload the recipes from Parse cloud
            [[NSNotificationCenter defaultCenter] postNotificationName:@"getPerfiles" object:self];
